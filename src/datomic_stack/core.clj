  ...
  (let [user-name (get @out->user-name out)]
    (if (logged-in? out->user-name out)
      (case cmd
        ...
        (async/put! out {:user/logged-in? true :cmd.error/name (str cmd " command not found") }))
      (case cmd
        "register" (register db-conn kv-args out->user-name out)
        "login" (login db-conn kv-args out->user-name out)
        (async/put! out {:user/logged-in? false :cmd.error/name (str cmd " command not found") })
        ))))
...
(defn transact [db-conn
  ...
  (async/put! out {:tx/completed true}))
...
(defn logout [db-conn out->user-name out]
  (let [res (swap! out->user-name dissoc out)]
    (async/put! out {:user/logged-in? (contains? res out)})))
...
(defn login [db-conn
  ...
  (let [actual (d/pull (d/db db-conn)
    ...
    (async/put! out {:user/logged-in? logged-in?
             :user/name name})))
...
(defn register [db-conn
  ...
  (let [{:keys [:db/id] :as existing} (d/pull
                                       ...
                                       [:user/name name])]
    (if id
      (async/put! out {:user/logged-in? false
